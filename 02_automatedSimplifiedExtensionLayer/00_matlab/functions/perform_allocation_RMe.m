function Model = perform_allocation_RMe( Model, Amatrixforallocationfactors)
...
Atoallocate = Model.matrices.A.mean_values;
...
new_meta_data_processes = cell(size(Model.meta_data_processes,1),NumOutFlowsOfMultiOutProcess+NumSingleOrNonOutProcess);
new_meta_data_processes(:,1) = Model.meta_data_processes(:,1);
...
for i=1:n %for all processes
    if NumOutFlowsOfProcess(i)<=1 %is single-functional or non-output process 
        ...
        new_meta_data_processes(:,c+1) = Model.meta_data_processes(:,i+1); %save original metadata column,+1 due to 'rownames' in first column of meta_data_processes
        ...
        for j=1:NumOutFlowsOfProcess(i) %for all output flows of multi-func process
            ...
            new_meta_data_processes(:,(c+j-1)+1) = Model.meta_data_processes(:,i+1); %duplicate original metadata column
            
            process_name = Model.meta_data_processes{1,i+1};
            flow_name = Model.meta_data_flows{IdxOutFlows(j)+1,1};
...
Model.matrices.A.mean_values = A_alloc;
Model.matrices.B.mean_values = B_alloc;
Model.matrices.F.mean_values = F_alloc;
Model.meta_data_processes = new_meta_data_processes;
end
function [ add_manual_processes ] = get_process_adding_with_mb(path_alignments)

% get availible data files in scenario folder
    av_files=extractfield(dir(path_alignments),...
        'name')';
    
    if ismember('manual_adding_processes_mass_balance',av_files)
        
        adding_files=extractfield(dir([path_alignments,'\manual_adding_processes_mass_balance']),...
        'name')';
    
        counter=0;
        for i=3:length(adding_files)
            
            filename=fullfile([path_alignments,'\manual_adding_processes_mass_balance'],adding_files{i});

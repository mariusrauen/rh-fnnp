function [ Model,elementary_flows_to_delete,elementary_flow_matching ] = get_added_processes_V2( Model , process_adding , correspondanceFile, pathGlobalInput , ecoinventVersion )
...
oldEcoinventVersion = process_adding.meta_data_processes{end,2};
if ~strcmp(oldEcoinventVersion,ecoinventVersion)
    mappingFile = readtable(fullfile(pathGlobalInput,correspondanceFile),'Sheet',[oldEcoinventVersion,' - ',ecoinventVersion]);
    rowNames = readcell(fullfile(pathGlobalInput,correspondanceFile),'Sheet',[oldEcoinventVersion,' - ',ecoinventVersion]);
    mappingFile.Properties.VariableNames = rowNames(1,:);
    clear rowNames
end

process_adding.meta_data_elementary_flows = [process_adding.meta_data_elementary_flows,process_adding.meta_data_elementary_flows];
...
%            disp([process_adding.meta_data_elementary_flows(i,1)]);
       % create missing flow meta data
       newFlowName = mappingFile(and(strcmp(mappingFile.(['name_',oldEcoinventVersion]), process_adding.meta_data_elementary_flows{i,1}), ...
           and(strcmp(mappingFile.(['compartment_',oldEcoinventVersion]), process_adding.meta_data_elementary_flows{i,2}), ...
           strcmp(mappingFile.(['subcompartment_',oldEcoinventVersion]), process_adding.meta_data_elementary_flows{i,3}))),:);
       newFlowName = newFlowName(1,:);
       process_adding.meta_data_elementary_flows{i,1} = newFlowName.(['name_',ecoinventVersion]);
       process_adding.meta_data_elementary_flows{i,2} = newFlowName.(['compartment_',ecoinventVersion]);
       process_adding.meta_data_elementary_flows{i,3} = newFlowName.(['subcompartment_',ecoinventVersion]);
       if strcmp(process_adding.meta_data_elementary_flows{i,1},"")
...
if size(process_adding.B.mean_values ,1) > 0
    rows_to_delete = (process_adding.B.flows.row_indices == 0)';
    elementary_flows_to_delete = process_adding.meta_data_elementary_flows(rows_to_delete,3:6);
    process_adding.meta_data_elementary_flows = process_adding.meta_data_elementary_flows(~rows_to_delete,1:3);
...
%     Model.matrices.F.col_names   = [Model.matrices.F.col_names process_adding.processes.names(i)];   
    ...
    Model.meta_data_processes(:, end+1) = process_adding.meta_data_processes(1:end-1,i+1);
    ...
    for j = 1:size(process_adding.B.mean_values,1)
        ...
        % write data
        Model.matrices.B.mean_values(process_adding.B.flows.row_indices(j),end) = ...
           Model.matrices.B.mean_values(process_adding.B.flows.row_indices(j),end) + process_adding.B.mean_values(j,i);
    end

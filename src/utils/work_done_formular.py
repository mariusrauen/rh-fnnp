def work_done_formular(Model):
    ...
    flow_name_missing_formular = []  # Initialize an empty list for missing formular flows
    ...
    for i in range(1, len(Model['meta_data_flows'])):
        if not Model['meta_data_flows'][i][9]:  # Check if the 10th column (index 9) is empty
            work_to_do += 1
            counter += 1
            flow_name_missing_formular.append(Model['meta_data_flows'][i][0])  # Add the flow name to the list

    flow_name_missing_formular = list(flow_name_missing_formular)  # Make sure it's a list
    progress = (flow_amounts - work_to_do) / flow_amounts  # Calculate the progress

    return progress, flow_name_missing_formular

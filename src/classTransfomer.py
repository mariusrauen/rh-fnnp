...
class Transformer:
    ...
    """def prepare_for_regression(self, df_dict: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        '''
        Prepare dataframes for regression by converting all numeric columns to float type.
        
        Parameters:
        df_dict (Dict[str, pd.DataFrame]): Dictionary containing dataframes to process
        
        Returns:
        Dict[str, pd.DataFrame]: Dictionary containing cleaned dataframes with float values
        '''
        ...
        def clean_numeric_string(value):
            ...
            # Remove any spaces and replace decimal comma with point
        ...
        for key, df in df_dict.items():
            ...
            for column in df_cleaned.columns:
                if column not in ['datetime', 'date', 'time', 'ID']:  # Skip non-numeric columns
                    try:
                        # Apply the cleaning function
                        df_cleaned[column] = df_cleaned[column].apply(clean_numeric_string)
